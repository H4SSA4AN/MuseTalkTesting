...
async def upload_audio(request):
    """Upload an audio file and start inference directly"""
    global state, avatar_manager, streaming_handler, current_audio_path
    ...
    try:
        ...
        async for field in reader:
            if field.name == 'audio':
                ...
                fps = int(await field.text())
            elif field.name == 'batch_size':
                batch_size = int(await field.text())
        
        if not audio_file:
            return web.json_response({"success": False, "error": "No audio file provided"}, headers={
                "Access-Control-Allow-Origin": "*",
                "Access-Control-Allow-Methods": "POST, OPTIONS",
                "Access-Control-Allow-Headers": "*"
            })
        ...
        if not audio_file.filename.lower().endswith(('.wav', '.mp3', '.m4a', '.flac', '.ogg')):
            return web.json_response({"success": False, "error": "Invalid audio format. Supported: wav, mp3, m4a, flac, ogg"}, headers={
                "Access-Control-Allow-Origin": "*",
                "Access-Control-Allow-Methods": "POST, OPTIONS",
                "Access-Control-Allow-Headers": "*"
            })
        ...
        # Save uploaded audio file as Answer.wav (required by MuseTalk)
        audio_path = os.path.join(answers_dir, "Answer.wav")
        ...
        print(f"[Upload] Audio file saved: {audio_path}")
        ...
        try:
            ...
            print(f"[Upload] Audio duration: {audio_duration:.2f}s")
        except Exception as e:
            print(f"[Upload] Could not get audio duration: {e}")
            audio_duration = 15.0  # Default fallback
        
        # Reset state for new inference
        state.reset_inference_state()
        state.audio_length = audio_duration
        ...
        def start_inference_thread():
            ...
            print(f"[GPU Thread] Starting inference for uploaded audio (FPS: {fps}, Batch Size: {batch_size})")
            try:
        ...
        def start_processing_thread():
            print("[CPU Thread] Starting frame processor for uploaded audio.")
            try:
                while True:
                    frames_batch = frame_queue.get()
                    if frames_batch is None:  # Termination signal
                        break
                    ...
                    state.total_batches_expected = max(state.total_batches_expected, state.batches_processed)
                    
                    # Check if stream should be ready
                    streaming_handler._check_stream_progress()
                    
                    # Print progress
                    streaming_handler._print_batch_progress(
                        state.batches_processed,
                        state.total_batches_expected,
                        state.inference_start_time
                    )
                    
                    frame_queue.task_done()
                
                # Mark inference as complete
                state.set_inference_complete()
                print("[CPU Thread] Frame processing completed.")
        ...
        # Start both threads
        inference_thread = threading.Thread(target=start_inference_thread)
        ...
        return web.json_response({
            "success": True, 
            "message": "Audio uploaded and inference started",
        ...
        print(f"[Upload] Error processing audio upload: {e}")
        import traceback
        traceback.print_exc()
        return web.json_response({"success": False, "error": f"Upload failed: {str(e)}"}, headers={
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "POST, OPTIONS",
            "Access-Control-Allow-Headers": "*"
        })
